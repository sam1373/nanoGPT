#!/usr/bin/env python3
# train.py – nanoGPT variant with NSA
# Two-line fix:  ❶ every rank now participates in estimate_loss()
#               ❷ the pre-loop barrier runs only when DDP is active
...
# --------------------------------------------------------------------------
# configuration (identical to the one you pasted)
# --------------------------------------------------------------------------
CFG = dict(
    out_dir='out',
    eval_interval=1000,
...
# --------------------------------------------------------------------------
# distributed / SLURM helpers (unchanged)
# --------------------------------------------------------------------------
...
model_args = dict(
    ...
    nsa_block_size=nsa_block_size, nsa_topk=nsa_topk, nsa_num_kv_heads=nsa_num_kv_heads,
...
if init_from == 'resume' and ckpt_files:
    ...
    model = GPT(GPTConfig(**model_args)) if init_from == 'scratch' else GPT.from_pretrained(init_from, dict(dropout=dropout))
...
def eval_ruler(tasks, verbose=False):
    ...
    for tname, samples in tasks.items():
        ...
        prefill_per_tok, decode_per_tok = [], []
        for i, s in enumerate(samples):
            ...
            if device_type == 'cuda': torch.cuda.synchronize(device)
            t0 = time.time()
            with ctx: _ = model(idx)                # prefill
            if device_type == 'cuda': torch.cuda.synchronize(device)
            t1 = time.time()
            with ctx:
            ...
            prefill_per_tok.append(pre_ms / max(prompt_len, 1))
            decode_per_tok.append(dec_ms / max(gen_len, 1))

            gen = tokenizer.decode(out[0].tolist())[len(prompt):]
            ok  = any(t in gen for t in targets)
            if verbose:
                short_prompt = prompt[-40:].replace('\n', ' ')
                print(f'[{tname} #{i:02d}] …{short_prompt} | pre:{pre_ms:6.1f} ms '
                      f'dec:{dec_ms:6.1f} ms | gen:"{gen[:60]}" | exp:{targets} | '
                      f'{"✔" if ok else "✘"}')
            correct += ok
            total   += 1
        ...
        avg_pre_token = float(np.mean(prefill_per_tok)) if prefill_per_tok else 0.0
        avg_dec_token = float(np.mean(decode_per_tok))  if decode_per_tok  else 0.0
...
# --------------------------------------------------------------------------
# ### FIX #2: barrier only when DDP is active
# --------------------------------------------------------------------------
if ddp:
...
while True:
    ...
    # ------------------------------------------------------------------
    # ### FIX #1 – all ranks now enter estimate_loss() to match ALLREDUCE
    # ------------------------------------------------------------------
    if iter_num % eval_interval == 0:
        losses = estimate_loss()                 # collective op
        if master_process:
            ...
            if enable_wandb:
                wandb.log({"iter": iter_num,
                           ...
                           "val_loss"  : losses['val']}, step=iter_num)
            if losses['val'] < best_val_loss or always_save_checkpoint:
                best_val_loss = losses['val']
                save_ckpt()
            if ruler_eval_enabled and iter_num % ruler_eval_interval == 0:
                ruler_metrics = eval_ruler(ruler_tasks, ruler_verbose)
                for tname, (acc, pre_ms, dec_ms,
                            pre_tok, dec_tok) in ruler_metrics.items():
                    print(f"iter {iter_num} - {tname}: "
                          f"RULER acc {acc:.2f}% | pre {pre_ms:.1f} ms "
                          f"| dec {dec_ms:.1f} ms | pre/token {pre_tok:.2f} "
                          f"| dec/token {dec_tok:.2f}")
                    if enable_wandb:
                        wandb.log({
                            f"ruler_accuracy_{tname}": acc,
                            f"ruler_prefill_ms_{tname}": pre_ms,
                            f"ruler_decode_ms_{tname}": dec_ms,
                            f"ruler_prefill_ms_per_token_{tname}": pre_tok,
                            f"ruler_decode_ms_per_token_{tname}": dec_tok
                        }, step=iter_num)
        if ddp:
            torch.distributed.barrier()          # keep ranks in lock-step
